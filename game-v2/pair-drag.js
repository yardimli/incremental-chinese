...
export function bindPairDrag(source, { getTargets, onDrop, enabled = () => true, signal }) {
  ...
  const clear = () => {
    ghost?.remove();
    ghost = null;
    hover?.classList.remove('drop-ready');
    hover = null;
    source.classList.remove('pair-dragging');
    drag = null;
  };
  const hit = (x, y) =>
    getTargets().find((el) => {
    ...
    (e) => {
      ...
      if (!drag.moved) {
        ...
        ghost.style.cssText =
          ...
          'px!important;height:' +
          drag.rect.height +
          'px!important;margin:0!important;';
        document.body.append(ghost);
        source.classList.add('pair-dragging');
      }
      ...
      if (hover !== candidate) {
        hover?.classList.remove('drop-ready');
        hover = candidate;
        hover?.classList.add('drop-ready');
      }
    },
    { signal },
  );
  source.addEventListener(
    ...
    (e) => {
      ...
      clear();
      if (destination) onDrop(destination);
    },
    { signal },
  );
  source.addEventListener(
