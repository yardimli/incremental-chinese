...
(async () => {
  ...
  try {
    ...
    async function seed(mode) {
      await page.evaluate(async (mode) => {
        const app = await import('../game-v2/shared.js'),
          E = await import('../game-v2/engine.mjs');
    ...
    for (const name of ['sets', 'settings', 'restart', 'bonus', 'memory', 'home']) {
      await nav(name);
    ...
    await page.keyboard.press('Space');
    await page.waitForFunction(() => JSON.parse(localStorage.getItem('chinese-game-v2')).coins > 0);
    await nav('sets');
    await nav('home');
    assert.ok(
