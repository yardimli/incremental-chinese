...
const lessons = await Promise.all(
  manifest.sets.map(async (f) =>
    JSON.parse(await readFile(new URL('../lessons/' + f, import.meta.url))),
  ),
);
...
test('unanswered targets rotate without score, discovery, or mastery and can still be completed', () => {
  const s = E.freshState(lessons, 0, 1234),
    id = s.current.id;
  const before = {
    coins: s.coins,
    correct: { ...s.correct },
  ...
  assert.deepEqual(
    { coins: s.coins, correct: s.correct, seen: s.seen, difficulty: s.difficulty },
    before,
  );
...
test('persistent weapons survive hits until mastery and misses preserve targets', () => {
  const s = E.freshState(lessons, 0, 92);
  s.difficulty = 2;
  s.current = null;
  E.newPrompt(s, lessons);
  const before = [...s.weapons],
    id = s.current.id;
  const wrong = before.find((w) => w !== id),
    targets = [...s.current.targets];
  E.answerTap(s, lessons, wrong, id);
  assert.deepEqual(s.current.targets, targets);
  assert.deepEqual(s.weapons, before);
  E.answerTap(s, lessons, id, id);
  assert.deepEqual(s.weapons, before);
  if (s.stage === 'cardReward') E.continueCardReward(s);
  s.correct[id] = 2;
  E.newPrompt(s, lessons, id);
  assert.ok(!s.weapons.includes(id));
});
test('four weapons support two independently resolved targets and escapes do not master', () => {
  const s = E.freshState(lessons, 0, 77);
  s.difficulty = 4;
  s.current = null;
  s.arrivals = 2;
  E.newPrompt(s, lessons);
  assert.equal(s.current.targets.length, 2);
  const [a, b] = s.current.targets;
  E.answerTap(s, lessons, b, b);
  assert.equal(s.correct[b], 1);
  assert.ok(s.current.targets.includes(a));
  const coins = s.coins;
  E.skipTap(s, lessons, a);
  assert.equal(s.correct[a] || 0, 0);
