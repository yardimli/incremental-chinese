  ...
  try {
    ...
    for (let found = 0; found < total; found++) {
      ...
      const id = await page.evaluate(async () => {
        const { state } = await import('../game-v1/shared.js');
        const { matchBoard } = await import('../game-v1/engine.mjs');
      ...
      await page.waitForFunction(
        (count) =>
          JSON.parse(localStorage.getItem('chinese-village-v1')).matchFound.length === count,
        found + 1,
      );
      await page.waitForTimeout(800);
      if (await page.locator('#screen #continue').isVisible()) {
        const stage = await page.evaluate(
          () => JSON.parse(localStorage.getItem('chinese-village-v1')).stage,
        );
        if (stage === 'cardReward') {
    ...
    await page.waitForFunction(
      () => JSON.parse(localStorage.getItem('chinese-village-v1')).stage === 'setReward',
    );
