...
export function validateLessons(lessons) {
 ...
 for(const set of lessons.filter(x=>x.type==='pairs')){
  ...
  for(const pair of set.pairs){if(ids.has(pair.id))throw Error('Duplicate card');ids.add(pair.id);for(const key of ['traditional','simplified','pinyin','english'])if(!pair[key])throw Error('Missing '+key)}
 }
 for(const set of lessons.filter(x=>x.type==='sentences')){
  ...
  for(const q of set.sentences)if(q.tokens.some(id=>!ids.has(id)))throw Error('Unknown token');
 }
}
function prepareSentence(s,set){
 s.sentencePool=shuffle(s,set.sentences.map(x=>x.id));
 const prior=s.previousSentences?.[set.id]||[];
 s.sentenceDeck=[...s.sentencePool.filter(id=>!prior.includes(id)),...s.sentencePool.filter(id=>prior.includes(id))].slice(0,set.countPerRun);
 s.sentenceIndex=0;s.sentenceSolved=[];
}
export function freshState(lessons, now = Date.now(), seed = 42) {
  const s = {version:2, coins:0, prestige:0, rng:seed >>> 0, lastAccrual:now,
    completed:[], cards:{}, highestLevel:1, settings:{script:'traditional', display:'both',voice:'female',muted:false,englishTranslations:'until10'},
  ...
  s.previousSentences={};s.history={};V.ensureVillage(s);
  newPrompt(s, lessons);
  return s;
}
export function reconcileLessons(s,lessons){ V.ensureVillage(s);s.highestLevel=reachedLevel(s); }
export function levelAccuracy(s){
 const a=s.accuracy?.[s.setIndex]||{correct:0,total:0,attempt:1};
 return {...a,percent:a.total?100*a.correct/a.total:0,passed:!a.total||a.correct*2>=a.total};
}
function recordAnswer(s,correct){
 s.accuracy??={};
 const a=s.accuracy[s.setIndex]??={correct:0,total:0,attempt:1};
 a.total++;if(correct)a.correct++;
}
function repeatLevel(s,lessons){
 const previous=levelAccuracy(s);
 s.accuracy??={};s.accuracy[s.setIndex]={correct:0,total:0,attempt:previous.attempt+1,lastPercent:previous.percent};
 s.seen=[];s.correct={};s.difficulty=1;s.current=null;s.weapons=[];s.arrivals=0;
 s.matchOrder=[];s.matchFound=[];s.pageIndex=0;s.pendingReward=null;
 const set=lessons[s.setIndex];s.stage=set.type==='sentences'?'order':'tap';
 if(s.stage==='order')prepareSentence(s,set);else newPrompt(s,lessons);
}
...
export function payout(s,scope,weight,level=s.level||1){return Math.floor(V.base(level)*weight*permanent(s)*V.multiplier(s,scope))}
export function award(s,scope,weight,level){const n=payout(s,scope,weight,level);V.recordActive(s,n);return n}
export function accrue(s,now=Date.now()){const result=V.accrueVillage(s,now);s.lastAccrual=Math.max(s.lastAccrual,now);return result.coins}
export function advance(s,lessons){
 if(s.stage!=='gate')return false;
 const next=lessons[s.setIndex+1];
 if(next?.type!=='sentences'&&!next?.auxiliary&&V.gateStatus(s,s.level||1).some(g=>g.have<g.need))return false;
 if(!next){s.stage='finished';return true}
 s.setIndex++;s.level=next.level;s.seen=[];s.correct={};s.difficulty=1;s.current=null;s.weapons=[];s.arrivals=0;s.matchOrder=[];s.matchFound=[];s.pageIndex=0;
 s.highestLevel=reachedLevel(s);
 if(s.completed.includes(next.id)){s.stage='gate';advance(s,lessons);return true}
 s.stage=next.type==='sentences'?'order':'tap';
 if(s.stage==='order')prepareSentence(s,next);else newPrompt(s,lessons);
 return true;
}
...
export function answerTap(s, lessons, choice, expectedId) {
  ...
  if (correct) { s.correct[id] = (s.correct[id] || 0) + 1; award(s,'W',1); }
...
export function matchPair(s, lessons, left, right) {
  if (s.stage !== 'match' || !matchBoard(s).includes(left) || !matchBoard(s).includes(right) || s.matchFound.includes(left) || s.matchFound.includes(right)) return null;
...
export function claimSet(s,lessons){
 ...
 const set=lessons[s.setIndex];if(s.completed.includes(set.id))return false;
 if(!levelAccuracy(s).passed){repeatLevel(s,lessons);return true}
 s.level=set.level||1;collectSetReward(s,set,s.sentenceDeck);
 s.stage='gate';advance(s,lessons);return true;
}
function collectSetReward(s,set,deck){
 s.completed.push(set.id);s.history[set.id]=true;
 let points=award(s,'set',10,set.level||1);
 if(set.id==='L01'){const grant=V.catalogue().progression.settlementGrant;V.recordActive(s,grant);points+=grant}
 if(set.type==='sentences')s.previousSentences[set.id]=[...deck];
 return points;
}
export const isLocalDebugHost=hostname=>['localhost','127.0.0.1','[::1]','::1'].includes(hostname);
export function debugCompleteSet(s,lessons,index,hostname){
 ...
 const set=lessons[index];if(s.completed.includes(set.id))return {completed:false,points:0};
 const current=index===s.setIndex;
 let ids;
 if(set.type==='sentences'){
  if(current&&s.sentenceDeck.length===set.countPerRun)ids=[...s.sentenceDeck];
  else {const selection={...s};prepareSentence(selection,set);s.rng=selection.rng;ids=selection.sentenceDeck}
 }else ids=set.pairs.map(w=>w.id);
 s.cards[set.id]=[...ids];
 const points=collectSetReward(s,set,ids);
 s.highestLevel=reachedLevel(s);
 if(current){
  s.pendingReward=null;s.current=null;
  if(set.type==='sentences'){s.sentenceDeck=[...ids];s.sentenceSolved=[...ids];s.sentenceIndex=ids.length}
  else {s.seen=[...ids];s.correct=Object.fromEntries(ids.map(id=>[id,2]));s.difficulty=4;s.matchOrder=[...ids];s.matchFound=[...ids]}
  s.level=set.level||1;s.stage='gate';advance(s,lessons);
 }
...
export function prestigeReset(s,lessons,now=Date.now()){
 ...
 const settings={...s.settings},prestige=s.prestige+1,history={...s.history},previousSentences={...s.previousSentences},highestLevel=reachedLevel(s);
 const fresh=freshState(lessons,now,s.rng);
 for(const key of Object.keys(s))delete s[key];
 Object.assign(s,fresh,{settings,prestige,history,previousSentences,highestLevel,resetToken:String(now)});return true;
}
