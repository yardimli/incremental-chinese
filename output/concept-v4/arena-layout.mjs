...
export function scatterCards(width, height, count, cardWidth, cardHeight, dictionaryBottom=0, random=Math.random) {
 const gap=14, edge=8;
 ...
 const maxX=width-edge-cardWidth, maxY=height-edge-cardHeight;
 for(let attempt=0;attempt<60;attempt++){
  const points=[];
  for(let trial=0;trial<300&&points.length<count;trial++){
   const p={x:edge+random()*(maxX-edge),y:top+random()*(maxY-top)};
   if(points.every(q=>Math.abs(p.x-q.x)>=cardWidth+gap||Math.abs(p.y-q.y)>=cardHeight+gap))points.push(p);
  }
  if(points.length===count)return points;
 }
 // Dense-layout fallback: random assignment and jitter within nonoverlapping cells.
 const cellWidth=(width-2*edge+gap)/columns,cellHeight=(height-edge-top+gap)/rows;
 const points=Array.from({length:count},(_,i)=>({
  x:edge+(i%columns)*cellWidth+random()*Math.max(0,cellWidth-cardWidth-gap),
  y:top+Math.floor(i/columns)*cellHeight+random()*Math.max(0,cellHeight-cardHeight-gap)
 }));
